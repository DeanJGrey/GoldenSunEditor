...
{
	class DataNumericUpDown : NumericUpDown
	{
		/*
		...
		public NumericUpDown doNud (Control panel, int x, int y, byte[] buffer, int address, int numOfBits)
		{
			...
			BackColor = Globals.color3;
			ForeColor = Globals.color0;
			...
			ValueChanged += new EventHandler(nUPChanged);
		...
		private void nUPChanged(object sender, EventArgs e)
		{
